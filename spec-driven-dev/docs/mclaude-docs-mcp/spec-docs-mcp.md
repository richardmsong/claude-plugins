...
- Entrypoint: `docs-mcp/src/index.ts`. On boot: resolves the **docs root** via `--root <dir>` CLI argument (preferred) or `process.cwd()` (fallback) — this is the parent of `docs/`. Separately discovers the **git root** by walking up from the docs root to find a `.git` directory (ADR-0038). Opens the SQLite DB at `<docsRoot>/.agent/.docs-index.db`. Passes `gitRoot` to `indexAllDocs` and `runLineageScan` so stored paths and git pathspecs are git-root-relative. If no `.git` is found, lineage scanning is skipped but content indexing proceeds using `docsRoot` as the path base. Starts `startWatcher` for the process lifetime; registers the four MCP tools. The docs directory is always `<docsRoot>/docs/`; `--docs-dir` is dashboard-only (ADR-0032).
