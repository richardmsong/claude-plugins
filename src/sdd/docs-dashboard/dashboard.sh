...
# ---- Launch Vite ----
# --host 0.0.0.0 binds all interfaces (needed for Tailscale/remote access).
# Subshell cd so the parent shell's PWD is preserved. exec avoids an extra layer.
(cd "$UI_DIR" && BACKEND_PORT="$BACKEND_PORT" exec bun x vite --port "$VITE_PORT" --strictPort --host 0.0.0.0) \
  >"$VITE_LOG" 2>&1 &
VITE_PID=$!
