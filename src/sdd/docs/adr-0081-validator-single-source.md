...
|----------|--------|-----------|
| Validator surface | Validators are methods on a concrete `validator` struct (unexported) satisfying an exported `Validator` interface declared in `src/sdd/spec/checks_interface.go`. The interface lists all 32 method signatures. Concrete type lives in `spec/checks.go` (authored by dev-harness, not invariant-compiler). Compile-time assertion `var _ Validator = (*validator)(nil)` is required in `checks.go` — Go's structural typing catches drift between interface and implementation at build time. | Interface + concrete + compile-time satisfaction check formalizes the contract surface in Go. Future readers see the full validator contract in one place (`checks_interface.go`). dev-harness can't silently drift from the interface — wrong signature or missing method fails the build. Methods-on-receiver is slight ceremony vs free functions but earns the compile-check + interface-documentation benefits. |
...
| Migration scope | All 31 affected invariants get supersession entries in this ADR's Invariant Delta. New invariant IDs reuse the old IDs' shape; the new ones are dotted-name siblings (e.g. `methodology.validator.registry_id_field` succeeding `methodology.registry.id_field`). One additional net-new invariant (`methodology.validator.test_shape_unit_only`) brings the total to 32 entries. | A single ADR captures the architectural shift atomically. Staging across multiple ADRs would mix old and new shapes mid-flight, which is exactly the inversion this ADR fixes. |
...
- **New**: `spec/checks.go` — concrete `validator` struct + 32 methods (one per registered invariant: 31 ports + 1 AST-walker `CheckTestShapeUnitOnly`). Compile-time assertion `var _ Validator = (*validator)(nil)`. **(in scope, dev-harness authors)**
...
- 31 supersession entries + 1 net-new entry under `### Added` in this ADR's delta. Each supersession carries `supersedes: <old_id>`; predecessors flip to `status: withdrawn` automatically via delta reconciliation. New IDs use the `methodology.validator.<area>_<thing>` prefix. New definitions are claims about validator behavior. **(in scope)**
...
var _ Validator = (*validator)(nil)  // compile-time assertion
...
- `src/sdd/spec/registry.yaml` (31 supersession entries + 1 net-new entry added; 31 predecessors flipped to `status: withdrawn`).
...
# 9. NEW: test-shape meta-check (1 entry) — net-new invariant
