...
**Why on-demand-only audit cadence Day-1.** Three options were on the table: on-demand only, quarterly+manual, nightly+scheduled. Picked on-demand because the audit chain is brand new — we don't yet know what kinds of divergence it surfaces, what the false-positive rate looks like, or how expensive a typical run is in real tokens. Scheduling a cadence before observing actual audit output bakes in assumptions that may not survive contact with reality. Triage owner is the master session that ran `/audit-invariants` — there is no separate ownership layer in single-developer plugin contexts. Scheduled/queued surfaces (nightly cron, dashboard queue) are deferred to ADR-0079 where they belong as ritual extensions.

**Why Go-only Day-1, multi-language deferred.** Two options: declare this ADR's umbrella as multi-language with Go as the reference, or scope strictly to Go-only and ship TS/Py/Rust under future ADRs. Picked Go-only. Multi-language Day-1 would force per-mechanism tool choices for TS (vitest vs jest vs node:test), Py (pytest vs unittest), Rust (cargo test + which property-testing lib) without any real consumer to validate against — speculative choices baked into the methodology before they have to be made. Go-only keeps this ADR's scope honest to what's actually implemented (`gopkg.in/yaml.v3`, `go test ./...`, `go vet ./...`) and lets each future language binding be its own decision space with its own consumer-driven Q&A.

**Why no Layer 0 in the audit chain, even though design-evaluator did more than decision-invariant-evaluator does.** design-evaluator's original scope was per-section ambiguity scan: endpoints fully specified, schemas complete, failure modes enumerated, security boundaries addressed, user flows unambiguous. Under invariant-driven dev, ~80% of that work moves to Layer 1 + /compile-invariants + structural invariants: endpoints/schemas/failure-modes/security boundaries are each registered invariants under Pattern B, and Layer 1 catches commitments hidden in narrative. The residual 20% — user-flow narrative ambiguity that has no corresponding commitment — is a *planning-time* discovery concern, not an audit-time check. Authoring is when you walk user flows and decide what contract each step commits to; by audit time, that discovery is done. So Layer 0 doesn't get restored; instead, /plan-feature's Step 3 Q&A is augmented to explicitly walk each user-flow step and prompt "what contract does this step commit to?" — turning each step into a candidate `### Added` entry. The audit chain stays at three layers (decision-↔-invariant, statement-↔-verifier, mutation tester); narrative quality is /plan-feature's job, not the auditors'.

**Why skill-content contracts defer to ADR-0080 instead of grep-verifiers in this ADR.** The /plan-feature user-flow-walkthrough discipline is a candidate for registration, but a grep-style verifier ("SKILL.md contains phrase X") proves text presence, not LLM behavior — a weak contract under invariant-driven dev's standard ("verifier is the executable form of the contract"). The real verifier for behavioral skill contracts is an *eval*: run the skill against canned inputs, score outputs with rubric or LLM-judge. That's a new mechanism the methodology hasn't declared yet, and adding it touches multiple concerns at once (eval framework choice, audit-time-only dispatch, cost/cadence, skill-vs-agent-vs-process scope). Worth its own ADR rather than smuggling a sub-feature into 0078. ADR-0080 will own the eval-as-verifier-mechanism decision plus the first skill invariants registered against it.
...
- ~~Differential regeneration cadence + owner~~ — **On-demand only Day-1; session-author triages.** See Decisions table and Decision history.
- ~~Language-binding generality~~ — **Go-only Day-1; multi-language deferred to per-language ADRs.** See Decisions table and Decision history.
- ~~Registry-coverage CI gate failure semantics~~ — **Resolved structurally:** verifiers use `t.Errorf` / `t.Fatalf`, so registry-coverage gaps fail `go test` and CI fails. No separate gate-semantics layer needed.
- ~~Methodology spec doc partition~~ — **Moot.** Under this methodology, `docs/**/spec-*.md` files are legacy (see Decision history line: "Why pre-existing spec-*.md docs become legacy"). The methodology's own commitment surface is this ADR + the registry + the verifiers, not a methodology spec doc.
- ~~Mutation testing for CI vs audit-only~~ — **Audit-only.** Component Changes pins mutation-tester as Layer 3, audit-only ("never a CI gate"). Resolved by the audit-chain decision.
- ~~Per-project stability threshold tunability~~ — **Deferred** until a second consumer with different needs surfaces. Defaults stay hardcoded in `StabilityOf`.

Resolved (during this ADR's authoring, batch 3):

- ~~Audit chain Layer 0 — generic ambiguity scan~~ — **Not restored.** ~80% of design-evaluator's original scope is now covered by `/compile-invariants` compile-gate + Layer 1 roundtrip + structural invariants. The residual concern (user-flow narrative ambiguity) is moved to `/plan-feature`'s authoring-time discipline: Step 3 Q&A walks each user-flow step explicitly and asks "what contract does this step commit to?" — every step prompts a candidate `### Added` entry. By audit time, narrative discovery is done; the audit chain only checks commitments. See Decision history.
