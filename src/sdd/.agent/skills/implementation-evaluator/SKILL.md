...
Spawns the `implementation-evaluator` agent for one or all components. The agent has no conversation context — it reads only spec docs and code.
...
```
/implementation-evaluator [component]
```
...
The agent saves results to `.agent/audits/impl-<component>-<YYYY-MM-DD>.md` and returns CLEAN or a gap list.
