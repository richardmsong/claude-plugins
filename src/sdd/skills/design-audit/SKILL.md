...
Record every fix and decision in the log. Every entry must include **what caused the gap** and **what was done about it** — this is required for traceability:
...
|---|-----|-------|-----------|------|
| 1 | <gap title> | <root cause: why the gap existed> | <what was changed in the doc> | factual |
| 2 | <gap title> | <root cause: why the gap existed> | <user chose option A: description> | decision |
| 3 | <gap title> | <root cause: why the gap existed> | <what was changed in the doc> | factual |
...
```
...
| # | Gap | Cause | Resolution | Type |
|---|-----|-------|-----------|------|
| 1 | git_identity_id delivery | ADR described the identity but never specified the delivery mechanism to the pod | Added MCProjectSpec field + GIT_IDENTITY_ID env var | factual |
| 2 | glab not in Alpine repos | ADR assumed `apk add glab` would work but glab is not packaged for Alpine | Changed to binary download from GitLab releases | factual |
| 3 | glab multi-account | ADR didn't address how multiple GitLab identities would be handled; glab CLI has no `auth switch` | User chose: one identity per GitLab host | decision |
...
