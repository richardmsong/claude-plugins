...
- **Bash/Edit/Write/Update**: dev-harness subagents run without supervision and cannot prompt for permission
...
```
SDD config refreshed.{{ if eq .Platform "factory" }} AGENTS.md{{ else }} CLAUDE.md{{ end }} updated to latest workflow rules.
```
