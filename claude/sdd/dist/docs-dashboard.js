...
function resolveDocsRoot(rawRoot, projectDir, cwd) {
  if (rawRoot !== null) {
...
function openDb(dbPath) {
  ...
  try {
    ...
    db.exec(SCHEMA_SQL);
    const row = db.query("SELECT value FROM metadata WHERE key = 'schema_version'").get();
    if (!row) {
...
// docs-mcp/src/content-indexer.ts
import { statSync, readFileSync, readdirSync, existsSync as existsSync2, lstatSync } from "fs";
import { join, relative } from "path";

// docs-mcp/src/parser.ts
var STATUS_RE = /^\*\*Status\*\*:\s*(draft|accepted|implemented|superseded|withdrawn)\s*$/i;
var STATUS_HISTORY_MARKER_RE = /^\*\*Status history\*\*:\s*$/i;
var HISTORY_LINE_RE = /^\s*-\s*(\d{4}-\d{2}-\d{2}):/;
...
function indexFile(db, filePath, repoRoot) {
  let stat;
  try {
  ...
  const existing = db.query("SELECT mtime FROM documents WHERE path = ?").get(docPath);
  ...
  db.run("BEGIN");
  try {
    db.run(`INSERT INTO documents(path, category, title, status, last_status_change, mtime)
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(path) DO UPDATE SET
         ...
         mtime = excluded.mtime`, [docPath, category, parsed.title, parsed.status, parsed.lastStatusChange, mtime]);
    const docRow = db.query("SELECT id FROM documents WHERE path = ?").get(docPath);
    db.run("DELETE FROM sections WHERE doc_id = ?", [docRow.id]);
    const insertSection = db.prepare(`INSERT INTO sections(doc_id, heading, content, line_start, line_end)
       VALUES (?, ?, ?, ?, ?)`);
...
function removeFile(db, filePath, repoRoot) {
  const docPath = relative(repoRoot, filePath).replace(/\\/g, "/");
  db.run("DELETE FROM documents WHERE path = ?", [docPath]);
}
function indexAllDocs(db, docsDir, repoRoot) {
  ...
  const allIndexed = db.query("SELECT path FROM documents WHERE path LIKE ?").all(`${relDocsDir}/%.md`);
...
// docs-mcp/src/lineage-scanner.ts
...
function isGitAvailable(repoRoot) {
...
function isRootCommit(repoRoot, commitHash) {
...
function getModifiedDocFiles(repoRoot, commitHash, relDocsDir) {
  const root = isRootCommit(repoRoot, commitHash);
  const args = root ? ["diff-tree", "--no-commit-id", "-r", "--name-only", "--root", commitHash, "--", `${relDocsDir}/*.md`] : ["diff-tree", "--no-commit-id", "-r", "--name-only", commitHash, "--", `${relDocsDir}/*.md`];
...
function getFileAtCommit(repoRoot, commitHash, filePath) {
  return git(repoRoot, ["show", `${commitHash}:${filePath}`]);
}
function parseDiffHunks(diffOutput, relDocsDir) {
...
function getCommitDiffHunks(repoRoot, commitHash, relDocsDir) {
  const root = isRootCommit(repoRoot, commitHash);
  const args = root ? ["diff-tree", "-p", "--root", commitHash, "--", `${relDocsDir}/*.md`] : ["diff", `${commitHash}~1..${commitHash}`, "--", `${relDocsDir}/*.md`];
  const out = git(repoRoot, args);
  if (!out)
    return new Map;
  return parseDiffHunks(out, relDocsDir);
}
function touchedSections(hunks, boundaries) {
...
function upsertLineage(db, docA, headingA, docB, headingB, commitHash) {
  db.run(`INSERT INTO lineage(section_a_doc, section_a_heading, section_b_doc, section_b_heading, commit_count, last_commit)
     VALUES (?, ?, ?, ?, 1, ?)
     ON CONFLICT(section_a_doc, section_a_heading, section_b_doc, section_b_heading)
     DO UPDATE SET commit_count = commit_count + 1, last_commit = excluded.last_commit`, [docA, headingA, docB, headingB, commitHash]);
}
function isAdrFile(filePath) {
  const basename = filePath.split("/").pop() ?? filePath;
  return basename.startsWith("adr-") && basename.endsWith(".md");
}
function runLineageScan(db, repoRoot, docsDir) {
  const relDocsDir = relative2(repoRoot, docsDir).replace(/\\/g, "/");
  ...
  const lastRow = db.query("SELECT value FROM metadata WHERE key = 'last_lineage_commit'").get();
...
function processCommitForLineage(db, repoRoot, commitHash, relDocsDir) {
  const modifiedFiles = getModifiedDocFiles(repoRoot, commitHash, relDocsDir);
  for (const filePath of modifiedFiles) {
    db.run("UPDATE documents SET commit_count = commit_count + 1 WHERE path = ?", [filePath]);
  }
  ...
  for (const filePath of modifiedFiles) {
    ...
    const boundaries = parsed.sections.map((s) => ({
      heading: s.heading,
  ...
  for (let i = 0;i < fileData.length; i++) {
    for (let j = i + 1;j < fileData.length; j++) {
      const a = fileData[i];
      const b = fileData[j];
      if (!isAdrFile(a.filePath) && !isAdrFile(b.filePath))
        continue;
...
`);
  ...
  for (const raw of rawLines) {
    ...
    if (last && last.commitHash === raw.commitHash && last.lineEnd === raw.lineNum - 1) {
...
function blameFile(db, repoRoot, filePath) {
  const relPath = relative3(repoRoot, filePath).replace(/\\/g, "/");
  const docRow = db.query("SELECT id FROM documents WHERE path = ?").get(relPath);
  if (!docRow) {
    return;
  }
  const docId = docRow.id;
  db.run("DELETE FROM blame_lines WHERE doc_id = ?", [docId]);
  const output = git2(repoRoot, ["blame", "--porcelain", filePath]);
  if (output === null) {
    return;
  }
  const blamed = parsePorcelain(output);
  const insert = db.prepare(`INSERT INTO blame_lines (doc_id, line_start, line_end, "commit", author, date, summary)
     VALUES (?, ?, ?, ?, ?, ?, ?)`);
  for (const b of blamed) {
    insert.run(docId, b.lineStart, b.lineEnd, b.commitHash, b.author, b.date, b.summary);
  }
}
function runBlameScan(db, repoRoot, docsDir) {
  const relDocsDir = relative3(repoRoot, docsDir).replace(/\\/g, "/");
  const allDocs = db.query("SELECT id, path FROM documents").all().filter((d) => d.path.startsWith(relDocsDir + "/"));
...
// docs-mcp/src/watcher.ts
import { existsSync as existsSync3 } from "fs";
import { join as join2, relative as relative4 } from "path";
var DEBOUNCE_MS = 100;
var POLL_INTERVAL_MS = 5000;
function startWatcher(db, docsDir, repoRoot, onReindex) {
  ...
  const runReindex = (event, filename) => {
    if (stopped)
      return;
    const changedPaths = [];
    if (filename && filename.endsWith(".md")) {
      const fullPath = join2(docsDir, filename);
      try {
        const reindexed = indexFile(db, fullPath, repoRoot);
        if (reindexed) {
          const relPath = relative4(repoRoot, fullPath).replace(/\\/g, "/");
      ...
      try {
        ...
        for (const p of reindexed) {
          changedPaths.push(p);
          const fullPath = join2(repoRoot, p);
          try {
            blameFile(db, repoRoot, fullPath);
          } catch (err) {
  ...
  try {
    if (!existsSync3(docsDir)) {
      ...
      const fsWatch = __require("fs").watch;
...
function boot(docsRoot, dbPath, onReindex) {
  const gitRoot = findGitRoot(docsRoot);
  if (!gitRoot) {
    console.warn(`[dashboard] No .git directory found walking up from ${docsRoot}; lineage and blame scanning disabled`);
  }
  const resolvedDbPath = dbPath ?? join3(docsRoot, ".agent", ".docs-index.db");
  const db = openDb(resolvedDbPath);
  const docsDir = join3(docsRoot, "docs");
  try {
...
var GetSectionSchema = exports_external.object({
  doc_path: exports_external.string().describe("Document path relative to repo root (e.g. docs/adr-2026-04-10-k8s-integration.md)"),
  heading: exports_external.string().describe("Section heading text (e.g. Component Changes)")
});
var GetLineageSchema = exports_external.object({
  doc_path: exports_external.string().describe("Document path relative to repo root (e.g. docs/spec-state-schema.md)"),
...
function getLineage(db, args) {
  ...
  if (!heading) {
    return db.query(`SELECT
         ...
         ORDER BY commit_count DESC`).all(doc_path);
  }
  return db.query(`SELECT
       ...
       ORDER BY l.commit_count DESC`).all(doc_path, heading);
}
function listDocs(db, args) {
  ...
  const docs = db.query(sql).all(...params);
  return docs.map((doc) => {
    const sections = db.query("SELECT heading, line_start, line_end FROM sections WHERE doc_id = ? ORDER BY line_start").all(doc.id);
    return {
      doc_path: doc.path,
      title: doc.title,
...
function readRawDoc(repoRoot, docPath) {
  const absRepoRoot = resolve2(repoRoot);
  const absPath = resolve2(absRepoRoot, docPath);
...
// docs-dashboard/src/graph-queries.ts
function globalGraphQuery(db) {
  const nodes = db.query(`SELECT path, title, category, status, commit_count FROM documents`).all();
  const rawEdges = db.query(`SELECT
       ...
       GROUP BY MIN(section_a_doc, section_b_doc), MAX(section_a_doc, section_b_doc)`).all();
...
function localGraphQuery(db, focus) {
  const rawEdges = db.query(`SELECT
         MIN(section_a_doc, section_b_doc) AS from_path,
         MAX(section_a_doc, section_b_doc) AS to_path,
         SUM(commit_count)                 AS count,
       ...
       GROUP BY MIN(section_a_doc, section_b_doc), MAX(section_a_doc, section_b_doc)`).all(focus, focus);
  const edges = rawEdges.map((r) => ({
    from: r.from_path,
    to: r.to_path,
    count: r.count,
  ...
  const nodes = db.query(`SELECT path, title, category, status, commit_count
       FROM documents
       WHERE path IN (${placeholders})`).all(...paths);
  return { nodes, edges };
}
...
function handleBlame(db, repoRoot, url) {
  const docPath = url.searchParams.get("doc");
  if (!docPath) {
    return badRequest("Missing required query param: doc");
  }
  ...
  const blameRows = db.query(`SELECT line_start, line_end, "commit", author, date, summary
       FROM blame_lines
       WHERE doc_id = ?
       ORDER BY line_start`).all(docId);
  const blocks = [];
  for (const row of blameRows) {
    const coModified = db.query(`SELECT d.path AS doc_path, d.title, d.status
         FROM blame_lines bl
         JOIN documents d ON d.id = bl.doc_id
         WHERE bl."commit" = ? AND bl.doc_id != ?
         GROUP BY d.id`).all(row.commit, docId);
    blocks.push({
...
function handleBlameOnDemand(db, repoRoot, docPath, docId, since, ref) {
  ...
  const result = spawnSync3("git", gitArgs, {
    cwd: repoRoot,
    encoding: "utf-8",
    maxBuffer: 20 * 1024 * 1024
  });
...
function parsePorcelainSimple(output) {
  const lines = output.split(`
`);
  const rawLines = [];
  const commitCache = new Map;
    ...
    if (headerMatch) {
      ...
      while (i < lines.length && !lines[i].startsWith("\t")) {
        ...
        if (meta.startsWith("author "))
          currentAuthor = meta.slice("author ".length).trim();
          ...
          currentSummary = meta.slice("summary ".length).trim();
        i++;
      }
...
function findUncommittedLines(repoRoot, docPath, blameRows) {
  const absPath = `${repoRoot}/${docPath}`;
  const result = spawnSync3("wc", ["-l", absPath], { encoding: "utf-8" });
  if (result.status !== 0)
    return [];
...
function handleDiff(repoRoot, url) {
  const docPath = url.searchParams.get("doc");
  ...
  if (!docPath)
    return badRequest("Missing required query param: doc");
  if (!commit)
    return badRequest("Missing required query param: commit");
  if (!lineStartParam)
    return badRequest("Missing required query param: line_start");
  if (!lineEndParam)
    return badRequest("Missing required query param: line_end");
  ...
  const absPath = `${repoRoot}/${docPath}`;
  const result = spawnSync3("git", ["show", commit, "--", absPath], {
    cwd: repoRoot,
    encoding: "utf-8",
    maxBuffer: 20 * 1024 * 1024
  });
...
function extractHunks(gitShowOutput, lineStart, lineEnd) {
...
function buildStartupBanner(port, ifaces = networkInterfaces()) {
  const lines = [`Dashboard ready:`, `  http://127.0.0.1:${port}/`];
...
async function main() {
  ...
  let stopWatcher;
  try {
    ({ gitRoot, db, stopWatcher } = boot(docsRoot, dbPath, (changed) => {
  ...
  const server = Bun.serve({
    ...
    error(err) {
      if (err.code === "EADDRINUSE") {
        console.error(`Error: port ${port} is in use. Use --port <n> or stop the other process.`);
        process.exit(1);
      }
